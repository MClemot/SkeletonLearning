...
#creating Fertility dir if it does not exist
if not os.path.exists('Fertility'):
    os.makedirs('Fertility')
...
if len(sys.argv) > 1:
    for shape in objects:
        ...
        filename = "fertility_noise/{}.obj".format(s)
        filename_normals = "fertility_noise/{}_normals.obj".format(s)
        ...
        torch.save(net, "NetworksObj/net_{}_{}_{}_{}.net".format(npl, dep, activation+("" if tv else "NoTV"), s))
        to_obj(vertices*M, "Fertility/cvsk_{}_{}.obj".format("Sine", s), lines=edges, tri=triangles)
        to_obj(upts*M, "Fertility/unif_points_{}_{}.obj".format("Sine", s))
        to_obj(skpts*M, "Fertility/skeletal_points_{}_{}.obj".format("Sine", s))
        ...
        torch.save(net, "NetworksObj/net_{}_{}_{}_{}.net".format(npl, dep, "igr", s))
        to_obj(vertices*M, "Fertility/cvsk_{}_{}.obj".format("igr", s), lines=edges, tri=triangles)
        to_obj(upts*M, "Fertility/unif_points_{}_{}.obj".format("igr", s))
        to_obj(skpts*M, "Fertility/skeletal_points_{}_{}.obj".format("igr", s))

        vertices, edges, triangles, net, skpts, upts = build_siren_neural_skeleton(pc, nc, npl=npl, dep=dep, delta=delta, time_limit=time_limit,plot=True)
        torch.save(net, "NetworksObj/net_{}_{}_{}_{}.net".format(npl, dep, "siren", s))
        to_obj(vertices*M, "Fertility/cvsk_{}_{}.obj".format("siren", s), lines=edges, tri=triangles)
        to_obj(upts*M, "Fertility/unif_points_{}_{}.obj".format("siren", s))
        to_obj(skpts*M, "Fertility/skeletal_points_{}_{}.obj".format("siren", s))
...
for shape in objects:
    ...
    r = Rotation.from_euler("xyz",[0, 135,0],degrees=True)
    arr = 1.5*r.as_matrix()

    rotmatrix = torch.Tensor(arr)

    net = torch.load("NetworksObj/net_{}_{}_{}_{}.net".format(npl, dep, "Sine", s))
    ...
    net1 = torch.load("NetworksObj/net_{}_{}_{}_{}.net".format(npl, dep, "igr", s))
    ...
    net2 = torch.load("NetworksObj/net_{}_{}_{}_{}.net".format(npl, dep, "siren", s))
