...
from siren_nn import optimize_siren_neural_sdf
...
def build_siren_neural_skeleton(pc1, nc1, npl=64, dep=6, delta=0.06, scaling=1.5,
                          lambda_pc = 100, lambda_eik=200, lambda_f=100, resampling=True,
                          trainednet=None, time_limit=120, scaleshape=True,plot=False):
    ...
    if trainednet != None:
        ...
        net = torch.load("Pretrained/pretrained_{}_{}_Sine.net".format(npl, dep))
        
        print("\n##### Optimizing the Siren neural sdf")
        
        optim = torch.optim.Adam(params=net.parameters(), lr=1e-3)# lr=2e-5)
        nepochs=5000
        
        t = time.time()
        try:
            optimize_siren_neural_sdf(net, optim, pc, nc,
                                batch_size=25000, pc_batch_size=25000, 
                                epochs=nepochs, lambda_pc = lambda_pc,
                                lambda_eik=lambda_eik, lambda_f=lambda_f,
                                plot_loss=plot)
