...
param = argv[0]
name = "fertility_v{}r".format(param)
meshPath2 = '../fertility_noise/{}.obj'.format(name)

if len(argv) > 1:
    method = argv[1]
    if method == 'coverage':
        endpng = 'cov'
        meshPath = '../Fertility/cvsk_coverage_{}.obj'.format(name)
    elif method == 'Sine':
        endpng = 'Sine'
        meshPath = '../Fertility/cvsk_Sine_{}.obj'.format(name)
    elif method == 'siren':
        endpng = 'siren'
        meshPath = '../Fertility/cvsk_siren_{}.obj'.format(name)
    elif method == 'igr':
        endpng = 'igr'
        meshPath = '../Fertility/cvsk_igr_{}.obj'.format(name)
    elif method == 'mcs':
        endpng = 'mcs'
        meshPath = f"../MCS/Results/{name}.skel.obj"
    elif method == 'vc':
        endpng = 'vc'
        meshPath = f"../Fertility/{name}_vc_thinned0.02.ply"

print(endpng)

outputPath = os.path.join(cwd, './{}{}.png'.format(name,endpng))

imgRes_x =  2048 # recommend > 1080 
imgRes_y =  2048 # recommend > 1080 
numSamples = 200 # recommend > 200
location = (0,0,0.85242) # (GUI: click mesh > Transform > Location)
rotation = (109,0.761,-71.2) # (GUI: click mesh > Transform > Rotation)
scale_unif = 0.75
...
gray = (0.75, 0.75, 0.75, 0.1)
meshColor2 = bt.colorObj(gray)
ptSize2 = 0.01
bt.setMat_pointCloud(mesh2, meshColor2, ptSize2, alpha=0.1, transmission=1)
