...
name = "guitar"
outputPath = os.path.join(cwd, './{}_skelpts.png'.format(name))
meshPath = '../ResultsObj/skeletal_points_Sine_{}.obj'.format(name)
...
location = (0.58312, 0, 0.99799) # (GUI: click mesh > Transform > Location)
rotation = (45.8, -6.74, 83.1) # (GUI: click mesh > Transform > Rotation)
scale_unif = 1
...
def make_spheres(objects, context, obj, scale=0.02):
    # Create a material
...
def make_spheres_pc(objects, context, obj, scale=0.005):
    # Create a material
    ...
    # Assign the color
    principled.inputs['Base Color'].default_value = (0.5,0.5,0.5,0.4)
...
red = (1, 0, 0, 1)
meshColor = bt.colorObj(red)
ptSize = 0.02
bt.setMat_pointCloud(mesh, meshColor, ptSize, 1, 0.5)
...
ptSize2 = 0.01
bt.setMat_pointCloud(mesh2, meshColor2, ptSize2, 0.4, 1)
